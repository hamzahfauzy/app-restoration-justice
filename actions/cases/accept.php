...
if(request() == 'POST')
{
    ...
    $message = "Perkara dengan judul ".$edit->title." telah di setujui";
    foreach($contacts as $contact)
    {
        Fonnte::send($contact->phone, $message);
    }

    set_flash_msg(['success'=>'Perkara berhasil diterima']);
    header('location:'.routeTo('cases/view',['id'=>$data->id]));
}

Page::set_title('Terima Perkara | '.$data->title);
